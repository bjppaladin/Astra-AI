...
export async function fetchLicensedUsers(accessToken: string): Promise<any[]> {
  const skuData = await graphFetch(
    accessToken,
    `${GRAPH_BASE}/subscribedSkus?$select=skuId,skuPartNumber,prepaidUnits,consumedUnits`
  );
  const skus: SubscribedSku[] = skuData.value || [];

  const skuIdToInfo: Record<string, { name: string; cost: number }> = {};
  for (const sku of skus) {
    const mapped = SKU_MAP[sku.skuPartNumber];
    if (mapped) {
      skuIdToInfo[sku.skuId] = mapped;
    } else {
      skuIdToInfo[sku.skuId] = { name: sku.skuPartNumber, cost: 0 };
    }
  }

  let allUsers: GraphUser[] = [];
  let nextLink: string | null =
    `${GRAPH_BASE}/users?$select=id,displayName,userPrincipalName,department,assignedLicenses,accountEnabled&$filter=assignedLicenses/$count ne 0&$count=true&$top=999`;
...
export async function fetchMailboxUsage(accessToken: string): Promise<Map<string, { usageGB: number; maxGB: number }>> {
  const csvData = await graphFetch(
    accessToken,
    `${GRAPH_BETA}/reports/getMailboxUsageDetail(period='D7')?$format=text/csv`,
    true
  );
...
export async function fetchM365Data(accessToken: string): Promise<any[]> {
  const [users, mailboxMap] = await Promise.all([
    ...
    fetchMailboxUsage(accessToken).catch(() => new Map()),
  ]);
  ...
  return users.map((user) => {
    ...
    return {
      ...
      status: mailbox
        ? mailbox.usageGB / mailbox.maxGB > 0.9
          ? "Critical"
          : mailbox.usageGB / mailbox.maxGB > 0.7
            ? "Warning"
            : "Active"
        : "Active",
    };
